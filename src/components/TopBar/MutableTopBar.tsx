...
export default function MutableTopBar() {
  const [handleTopbar, setHandleTopbar] = useState<boolean>(true);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollTop > 0) {
        setHandleTopbar(false);
      } else {
        setHandleTopbar(true);
      }
    };
  ...
  return (
    <div>
      <div
        className={`w-full h-14 lg:h-16 fixed flex items-center justify-between px-4 lg:px-8 transition duration-200 border-b
        ${menu ? "-translate-y-full md:translate-y-0" : "translate-y-0"} 
        ${
          handleTopbar
            ? "bg-transparent border-transparent"
            : "bg-zinc-900 border-zinc-100/40"
        }`}
      >
